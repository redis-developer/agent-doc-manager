  ...
  switch (command.command) {
    ...
    case "modify_extracted_text":
      if (command.pageName && command.instructions) {
        ...
        logger.info(`Modify extracted text tool result: ${result}`, {
          userId,
          file: result,
        });
        ...
        logger.warn(
          `No pageName or instructions provided for modify_extracted_text command`,
          {
            userId,
          },
        );
    ...
    case "view_extracted_text":
      if (command.pageName) {
        ...
        logger.info(`View extracted text tool result: ${files[0].url}`, {
          userId,
          file: files[0],
        });
        response = files[0].content;
      } else {
        logger.warn(`No pageName provided for view_extracted_text command`, {
          userId,
        });
        response = "You need to provide a page name to view the text.";
      }
      break;
